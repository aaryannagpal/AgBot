...
Joining the two side chassis with the other two chassis, we obatain the same structure as the one shown in the image in the Project Description section.
...
Additionally, please ensure that the ```serial``` library is installed on the Raspberry Pi. If not, we first uninstall the old version by using the following command:
```bash
pip3 uninstall serial
```
and then install the newer version by using the following command:
```bash
